...
def add_order():
    ...
    if not customer:
        ...
        quantity_var.set(1)
        customer_entry.delete(0, tk.END)  # Clear name entry for next customer
        update_order_list()
...
def update_order_list():
    ...
    for order in orders:
        ...
        # Insert customer name on line 1, and order details on line 2
        order_list.insert(tk.END, f"Customer: {customer}")
        order_list.insert(tk.END, f"  └─ {quantity}x {size} {pizza} (${cost})")

        # Add a subtle background color shift to pair the blocks visually
        current_end = order_list.size()
        order_list.itemconfig(current_end - 1, fg="#555555")

    if orders:
        order_list.insert(tk.END, "")  # Blank spacer line before total
        order_list.insert(tk.END, f"Total cost: ${total_cost}")

        # Highlights the final Total Cost row cleanly
        order_list.itemconfig(tk.END, bg=TOTAL_BG)


def handle_click(event):
    """Ensures both the name line and details line highlight together when clicked."""
    selection = order_list.curselection()
    if not selection:
        return

    index = selection[0]
    total_rows = order_list.size()

    # Block selecting the blank line or total line at the bottom
    if index >= total_rows - 2 if orders else False:
        order_list.selection_clear(0, tk.END)
        return

    # Clear individual selection highlighting first
    order_list.selection_clear(0, tk.END)

    # Determine which pair was clicked (Even index = Name, Odd index = Details)
    if index % 2 == 0:
        name_idx = index
        details_idx = index + 1
    else:
    ...
    # Force selection highlight across both linked items
    order_list.selection_set(name_idx, details_idx)


def delete_selected_pizza():
    """Removes the selected multi-line block item from the order list."""
    order_selection = order_list.curselection()
    if not order_selection:
        messagebox.showerror("Input Error", "Please select an order block to delete.")
        return

    # Find the first row indexed inside the selection matrix
    first_selected_index = order_selection[0]

    # Map the list box click location directly back to our underlying data array index
    data_index = first_selected_index // 2

    if data_index < len(orders):
        del orders[data_index]
        update_order_list()
...
quantity_menu["menu"].config(bg=QUANTITY_BG)

cost_display_var = tk.StringVar(root, value="Cost per pizza: $8")
...
order_cost_var = tk.StringVar(root, value="Order cost: $8")
...
# We use browse mode so only single items are selected by user interactions natively
order_list = tk.Listbox(root, width=45, height=12, selectmode=tk.BROWSE)
...
# CRITICAL BIND: Connects listbox clicks directly to our dynamic dual-highlighting handler function
order_list.bind("<<ListboxSelect>>", handle_click)
